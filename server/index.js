...
const DB_PATH = join(DATA_DIR, 'rsvp.sqlite');
...
app.post('/api/public/families/:slug/rsvp', (req, res) => {
  try {
    ...
    res.json({ block, familyName: fam.name, responsible: db.prepare('SELECT responsible FROM families WHERE id = ?').get(fam.id).responsible });
  } catch (e) {
    res.status(500).json({ error: String(e.message || e) });
  }
});
